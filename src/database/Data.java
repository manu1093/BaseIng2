...
public class Data extends Database implements SData{
    ...
    @Override
        ...
        public ArrayList<Azione> listaAction(int n){
            ArrayList <Azione>l=new ArrayList <Azione>();
            ResultSet r=this.getRsQuery("select * from azione ;");
        try {
            while(r.next())
                l.add(new Azione(r.getString("nome"),r.getString("link"),r.getString("immagine"),r.getString("messaggio")));
            while(l.size()>n)
                 l.remove((int)(Math.random()*l.size()));
        } catch (SQLException ex) {
            Logger.getLogger(Data.class.getName()).log(Level.SEVERE, null, ex);
        ...
        public void inserisciAction(Azione a) {
            PreparedStatement p;
        try {
            p = conn.prepareStatement("insert into azione values(?,?,?,?);");
              p.setString(1,a.getNome());
            p.setString(2,a.getLink());
            p.setString(3, a.getMessaggio());
            p.setString(4, a.getImmagine());
            p.execute();
        } catch (SQLException ex) {
            Logger.getLogger(Data.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        public void rimuoviAction(String a) {
        try {
            conn.createStatement().execute("delete from azione where nome=\'"+a+"\';");
        } catch (SQLException ex) {
            Logger.getLogger(Data.class.getName()).log(Level.SEVERE, null, ex);
        }
        }
        ...
        public static void main(String args[]) throws SQLException{
          ...
          // new Database().inserisciUtente(new Cliente("Via Verdi","16","Vicenza","4558523574121245", "12/18","524",123,"telefonia","paolo00","matitanera"));
           ...
           for(Prodotto p:h.keySet()){
            ...
            new Data().aggiungiAlCarrello(new Carrello("ag349512","gaetano78",5));
        }
